...
public class SorterAgent : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        rewardMode = (int)m_ResetParams.GetWithDefault("reward_mode", 0);
        m_StepCount = 0;
...

    {
        ...
        if (rewardMode == 1){ //smaller penalty if moving fast
            float speed = m_AgentRb.velocity.magnitude;
            AddReward(-0.01f / (speed + 0.1f)); 
        } else if (rewardMode == 2){ //bonus for early completion
            if (m_NextExpectedTileIndex == m_NumberOfTilesToSpawn){
                float timeBonus = 1f - ((float)m_StepCount / MaxStep)
                AddReward(timeBonus);
            }
        } else if (rewardMmode == 3){ // penalty for standing still or barely moving
            if (m_AgentRb.velocity.magnitude < 0.1f){
                AddReward(-0.005f);
            }
        } else if (rewardMode == 4){      // 1 & 2
            float speed = m_AgentRb.velocity.magnitude;
            AddReward(-0.01f / (speed + 0.1f)); 
